...
class PrescriptionViewSet(viewsets.ModelViewSet):
    ...
    def verify(self, request, pk=None):
        ...
        # [NEW] Trigger Notification for Doctor
        from .models import Notification
        if action_type in ['APPROVE', 'FLAG', 'REJECT']:
            status_map = {
                'APPROVE': 'Approved',
                'FLAG': 'Flagged/Refined',
                'REJECT': 'Rejected'
            }
            Notification.objects.create(
                recipient=prescription.doctor,
                prescription=prescription,
                title=f"Prescription {status_map.get(action_type)}",
                message=f"Your prescription for {prescription.patient.name} has been {status_map.get(action_type).lower()} by Pharmacist {request.user.username}. Comment: {comment}"
            )
    ...
    def audit(self, request):
        ...
        data = [{
            ...
            'patient_name': l.prescription.patient.name if l.prescription and l.prescription.patient else None
        } for l in logs]
